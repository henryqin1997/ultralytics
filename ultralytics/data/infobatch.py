...
class InfoBatch(Dataset):
    ...
    def __init__(self, dataset: Dataset, num_epochs: int,
        ...
        self.scores = torch.ones(len(self.dataset)) * 100
        self.weights = torch.ones(len(self.dataset))
        self.num_pruned_samples = 0
        self.cur_batch_index = None

    # def __getattribute__(self, name):
    #     raise NameError('HiThere')(f"__getattribute__ called for {name}")
    #     try:
    #         # Attempt to get the attribute normally
    #         return super().__getattribute__(name)
    #     except AttributeError:
    #         # If the attribute doesn't exist, call __getattr__
    #         warnings.warn(f"{name} not found, calling __getattr__")
    #         return self.__getattr__(name)

    def __getattr__(self, name):
        # Delegate the method call to the sub_component if it is not found in Wrapper
        # warnings.warn(f"Looking for {name} in Wrapper, delegating to self.dataset")
